...
export function ConsultaAdaptiveNeck({
  ...
  function handleNext() {
    ...
    if (issue) {
      onSectionError(formatValidationIssueMessage(issue, locale, localizeNeckLabel(issue.questionId, issue.message.replace(/^Responde:\s*/, "").replace(/\.$/, ""), locale)));
      return;
    }
    onSectionError(null);
      ...
      {urgent && currentSection !== "red_flags" && (
        <View style={styles.redBox}>
          <Text style={styles.redText}>
            {redFlagsDetectedLabel(locale)} {triggered.map((x) => localizeNeckOption(x, locale)).join(", ")}.{redFlagsUrgencyNote(locale)}
          </Text>
        </View>
      )}
