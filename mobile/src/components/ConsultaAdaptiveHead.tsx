...
export function ConsultaAdaptiveHead({
  ...
  function handleNext() {
    ...
    if (issue) {
      onSectionError(formatValidationIssueMessage(issue, locale, localizeHeadLabel(issue.questionId, issue.message.replace(/^Responde:\s*/, "").replace(/\.$/, ""), locale)));
      return;
    }
    onSectionError(null);
      ...
      {urgent && currentSection !== "red_flags" && (
        <View style={styles.redBox}>
          <Text style={styles.redText}>
            {redFlagsDetectedLabel(locale)} {triggered.map((x) => localizeHeadOption(x, locale)).join(", ")}.{redFlagsUrgencyNote(locale)}
          </Text>
        </View>
      )}
