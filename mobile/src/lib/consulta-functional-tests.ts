...
import { buildFunctionalProtocolPromptBlock, buildUniversalNonUrgentPathwayPrompt } from "./consulta-functional-protocols";
...
export const FUNCTIONAL_TEST_QUESTIONS: Record<FunctionalRegionId, string[]> = {
  ...
  thoracic: [
    "¿Puedes girar el tronco sentado a ambos lados sin dolor fuerte en costillas/espalda media? (SÍ/NO)",
    "Al inspirar hondo o toser, ¿duele una costilla o un punto concreto? (SÍ/NO)",
    "¿Puedes elevar ambos brazos y juntar omoplatos sin pinzamiento? (SÍ/NO)",
    "¿Puedes inclinarte de lado (derecha e izquierda) sin dolor fuerte en la espalda media? (SÍ/NO)",
    "¿Al girar/inclinar la cabeza empeora el dolor de espalda media o hay hormigueo en el brazo? (SÍ/NO)",
  ],
  lumbar: [
  ...
  pelvis: [
    "Al apoyar solo una pierna 20–30 s, ¿hay inestabilidad o dolor en ingle/nalga/sacro? (SÍ/NO)",
    "Tumbado, ¿duele al levantar una pierna estirada 20–30 cm? (SÍ/NO)",
    "Al subir un escalón, ¿aumenta el dolor en pelvis, nalga o sacro? (SÍ/NO)",
    "Al levantarte de una silla sin manos, ¿duele en un lado de la pelvis/nalga? (SÍ/NO)",
    "¿Duele también al cruzar piernas o ponerte el calcetín (más ingle que sacro)? (SÍ/NO)",
  ],
  hip: [
...
const REGION_ALIASES: { match: RegExp; id: FunctionalRegionId }[] = [
  ...
  { match: /torácic|toracic|dorsal|costilla|rib|espalda\s*media|mid\s*back|thoracic|entre\s*om[oó]platos/i, id: "thoracic" },
  { match: /pelvis|sacroil|sacro[\s-]?iliac|\bSIJ\b|cintura\s*p[eé]lvica/i, id: "pelvis" },
