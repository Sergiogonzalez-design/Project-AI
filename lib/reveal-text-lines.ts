...
export function buildRevealChunks(text: string): RevealChunk[] {
  ...
  while (i < text.length) {
    ...
    while (start < lineContentEnd) {
      ...
      // No sentence break — hard wrap on last whitespace in the window.
...
/** @deprecated Prefer buildRevealChunks + visibleTextFromChunks. Kept for callers. */
...
function findSoftBreak(window: string, minChars: number): number | null {
  // Prefer sentence boundaries: . ! ? … followed by space or end.
  const re = /[.!?…」』](?:["'”’)]+)?(?=\s|$)/g;
  ...
  while ((m = re.exec(window)) != null) {
    ...
    // Skip common abbreviations / decimals (e.g. "Dr.", "3.5")
    if (m[0] === "." && isLikelyAbbreviationOrDecimal(window, m.index)) continue;
    best = after;
  }
  if (best != null) {
    // Include following spaces so the next chunk starts cleanly.
    while (best < window.length && window[best] === " ") best += 1;
    return best;
  }

  // Secondary: semicolon or em-dash mid clause.
  const secondary = /[;:—–](?=\s)/g;
...
function isLikelyAbbreviationOrDecimal(window: string, dotIndex: number): boolean {
  ...
  if (before && /[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/.test(before)) {
    // Single-letter initials / short titles: "Dr.", "p.", "mm."
