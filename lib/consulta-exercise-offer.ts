/** Post-consult exercise / self-care offer — end of patient consulta flow. */

export type PostConsultExerciseStep = "offer" | "plan";

export function postConsultExerciseOfferSymptomContext(
  bodyArea: string,
  language: "es" | "en" = "es",
): string {
  const zone = bodyArea.trim() || (language === "en" ? "this case" : "este caso");
  if (language === "en") {
...
/** Patient wants the offered exercise / self-care plan. */
export function affirmsExerciseOffer(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (!t) return false;
  if (declinesExerciseOffer(text)) return false;
  if (
    /^(s[ií]+|yes|yeah|yep|ok|okay|vale|claro|perfecto|por favor|please|genial|adelante|dale|empecemos?)[\s!.]*$/i.test(
      t,
    )
  ) {
    return true;
  }
  return /\b(s[ií]|yes|quiero|me\s+gustar[ií]a|dame|prop[oó]n|prepara|hazme|env[ií]ame|cu[eé]ntame|tell\s+me|i\s+want|go\s+ahead)\b/i.test(
    t,
  );
}

/** Patient declines the exercise / self-care plan offer. */
export function declinesExerciseOffer(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (!t) return false;
  if (
    /^(no|nope|nah|no\s+gracias|gracias)[\s!.]*$/i.test(t) ||
    /\b(no\s+gracias|ahora\s+no|not\s+now|no\s+thanks|paso|prefiero\s+no|nada\s+gracias)\b/i.test(
      t,
    )
  ) {
    return true;
...
/** Build case summary text for post-consult AI calls. */
export function buildPostConsultCaseSummary(
  evaluations: { label: string; summary: string }[],
  fallbackAssistantTexts: string[],
): string {
  ...
  const trimmed = fallbackAssistantTexts.map((t) => t.trim()).filter(Boolean);
  if (trimmed.length > 0) {
    return trimmed.join("\n\n---\n\n");
  }
  return "";
